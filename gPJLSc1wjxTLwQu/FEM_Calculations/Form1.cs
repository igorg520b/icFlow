    ...
    public partial class Form1 : Form
    {
        ...
        private void glControl1_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (model3.mc.failedCZs != null && rprm.FailedCZvisible && rprm.CZsFill)
                model3.mc.failedCZs.RenderCZ_Fill(rprm.FailedCZColor, true);

            if (model3.mc.nonFailedCZs != null && rprm.DamagedCZvisible && rprm.CZsFill)
                model3.mc.nonFailedCZs.RenderCZ_Fill(rprm.DamagedCZColor, false);
            ...
            if (model3.mc.failedCZs != null && rprm.FailedCZvisible && rprm.CZsEdgesVisible)
                model3.mc.failedCZs.RenderCZ_Edges(rprm.CZsEdgesColor, rprm.CZsEdgeWidth, true);

            if (model3.mc.nonFailedCZs != null && rprm.DamagedCZvisible && rprm.CZsEdgesVisible)
                model3.mc.nonFailedCZs.RenderCZ_Edges(rprm.CZsEdgesColor, rprm.CZsEdgeWidth, false);
        ...
        private void takeScreenshotToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            string filename = $"{savePath}{model3.cf.StepNumber:0000}.png";
        ...
        {
            if (running) return;
            if (renderingAnimation)
            {
                renderingAnimation = false;
                timer1.Enabled = false;
                tssStatus.Text = "Aborted rendering";
                if (swReport != null) swReport.Close();
                return;
            }
            timer1.Interval = rprm.Delay;
            renderSavePath = model3.saveFolder + $"\\{rprm.renderFolder}\\";
            if (Directory.Exists(renderSavePath))
                Directory.Delete(renderSavePath, true);
            Directory.CreateDirectory(renderSavePath);
            renderingAnimation = true;
            frameIsReady = false;
            renderingFrame = 0;
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            {
                // save rendered frame
                tssStatus.Text = $"{mixingCoeff * 100:00}%, save {renderingFrame}, {frameTime} s";
                ...
                using (Bitmap bmp = new Bitmap(w, h))
                {
                    ...
                    bmp.RotateFlip(RotateFlipType.RotateNoneFlipY);
                    string filename = $"{renderSavePath}{renderingFrame:0000}.jpg";
