...
namespace icFlow
{
    public class MeshCollection
    {
            ...
            foreach (CZ cz in nonFailedCZs)
                ...
                foreach (Node nd1 in cz.vrts)
                    foreach (Node nd2 in cz.vrts)
                        if (!nd1.anchored && !nd2.anchored && nd2.altId >= nd1.altId) csrd.AddStatic(nd1.altId, nd2.altId);
        ...
        byte[] buffer;
        public void WriteImpacts(Stream str, Stream impactsDataStream)
        {
            BinaryWriter bw = new BinaryWriter(str);
            ...
            // write impacts separately
            BinaryWriter bwImpacts = new BinaryWriter(impactsDataStream);
            double[] collisionDetails = new double[12 * nCollisions];
            if (nCollisions != 0)
            {
                int buffer_size = 12 * sizeof(double) * nCollisions;
                if (buffer == null || buffer.Length < buffer_size) buffer = new byte[buffer_size];
                for (int i_im = 0; i_im < nCollisions; i_im++)
                {
                    for (int i = 0; i < 4; i++)
                    {
                        Node ni = allNodes[collisions[i_im + i * collision_stride]];
                        collisionDetails[i_im * 12 + i * 3 + 0] = ni.cx;
                        collisionDetails[i_im * 12 + i * 3 + 1] = ni.cy;
                        collisionDetails[i_im * 12 + i * 3 + 2] = ni.cz;
                    }
                }
                    Buffer.BlockCopy(collisionDetails, 0, buffer, 0, buffer_size);
//                bwImpacts.Write(buffer, 0, buffer_size);
            }
            bwImpacts.Flush();
        }
