...
namespace icFlow
{
    public class GPU_Functionality
    {
        ...
        const int INT_DATA_SIZE_ELEM = 24;
        const int FP_DATA_SIZE_CZ = 12;
        const int INT_DATA_SIZE_CZ = 52;
        ...
        int Sum(CUdeviceptr array_ptr, int size)
        {
            ...
            if (size <= 1024)
            {
                n_kerSum.BlockDimensions = new dim3(1024, 1, 1);
                n_kerSum.GridDimensions = new dim3(1, 1, 1);
            }
            else
            {
                int block_size = 1024;
                n_kerSum.BlockDimensions = new dim3(block_size, 1, 1);
                int ngrid = grid(size, block_size);
                if (ngrid > 512) ngrid = 512;
                n_kerSum.GridDimensions = new dim3(ngrid, 1, 1);
            }
            ...
            Parallel.For(0, mc.elasticElements.Length, i_elem =>
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        int pcsr_ij;
                        Node nj = elem.vrts[j];
                        if (!ni.anchored && !nj.anchored && (!prms.Symmetric || nj.altId >= ni.altId))
        ...
        void AllocateMemoryForLinearSystem()
        {
            ...
            if (g_dvals == null || g_dvals.Size < bcsr.dvalsSize)
            {
                if (g_dvals != null) g_dvals.Dispose();
                g_dvals = new CudaDeviceVariable<double>((int)(bcsr.dvalsSize * BCSR.overAllocate));
            }
            if (g_drhs == null || g_drhs.Size < bcsr.dxSize)
            {
                if (g_drhs != null) g_drhs.Dispose();
                g_drhs = new CudaDeviceVariable<double>(bcsr.dxSize);
            }
        ...
        public void AssembleElemsAndCZs()
        {
            ...
            if (cz_stride != 0)
            {
                ...
                kczCZForce.RunAsync(CUstream.NullStream, g_dcz.DevicePointer, g_icz.DevicePointer, 
                    ...
                    g_dvals.DevicePointer, g_drhs.DevicePointer, 
                    cf.TimeStep,
                    mc.nonFailedCZs.Length, cz_stride, nd_stride, (int)prms.czFormulaiton);
        ...
        {
            g_dvals.CopyToHost(bcsr.vals, 0, 0, bcsr.nnz * 9 * sizeof(double));
            g_drhs.CopyToHost(bcsr.rhs, 0, 0, bcsr.N * 3 * sizeof(double));
        }
...
//            Trace.WriteLine($"narrowList: {narrowList.Count}");
        ...
        public void collisionResponse()
        {
            ...
            Parallel.For(0, nImpacts, i_im => {
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    for (int j = 0; j < 4; j++)
                    {
                        ...
                        if (!ni.anchored && !nj.anchored && (!prms.Symmetric || nj.altId >= ni.altId)) pcsr_ij = ni.pcsr[nj.altId];
                        else pcsr_ij = -1; // ni is anchored => ignore
                        c_icr[i_im + cr_stride * (pcsr_offset + (i * 4 + j))] = pcsr_ij;
                    }
                }
            });
            g_icr.CopyToDevice(c_icr, 0, 0, sizeof(int) * cr_stride * 24);
            ...
            kCollisionResponseForce.Run(g_icr.DevicePointer, g_dn.DevicePointer, cf.TimeStep,
                g_dvals.DevicePointer, g_drhs.DevicePointer, nImpacts, cr_stride, nd_stride, prms.penaltyK, prms.DistanceEpsilon);
