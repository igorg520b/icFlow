...
namespace icFlow
{
    ...
    public class FrameInfo
    {
        ...
        public static void WriteCSV(List<FrameInfo> frames, string path)
        {
            ...
            sw.WriteLine($"Step,Stress,Strain,Force,DamageAndFail,Fail,TimeScaleFactorLog,Iterations, Time, CumulativeComputeTime");
            long ComputeTime = 0;
            for (int i = 0; i < frames.Count; i++)
            {
                ...
                sw.WriteLine($"{i},{f.stress},{f.strain},{f.IndenterForce},{damageAndFail},{fail},{Math.Log((double)f.TimeScaleFactor, 2) + 1},{f.IterationsPerformed}, {f.SimulationTime}, {ComputeTime}");
            }
            sw.Close();
        }
