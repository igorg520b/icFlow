...
extern "C" __global__ void kczCZForce(
	...
	const double h,
	const int nCZs, const int cz_stride, const int nd_stride, const int formulation) {
	...
	const int *pcsr = &icz[idx + cz_stride*PCSR_OFFSET_CZ];
	// use single precision
	double *A = _global_matrix;
	double *b = _global_rhs;
	...
	for (int i = 0; i < 6; i++)
	{
		...
		for (int j = 0; j < 6; j++)
		{
			int idx1 = pcsr[cz_stride * (i * 6 + j)];
			if (idx1 >= 0)
			{
				// write into csr.vals
				for (int k = 0; k < 3; k++)
					for (int l = 0; l < 3; l++)
						atomicAdd2(&A[idx1 * 9 + 3 * k + l], Keff[i * 3 + k][j * 3 + l]);
			}
		}
		// distribute rhs
		int idx2 = pcsr[cz_stride * (36 + i)];
		if (idx2 >= 0)
		{
			for (int k = 0; k < 3; k++)
