...
//            if (rprm.RenderText) RenderTextAndPlots();
            ...
            using (Bitmap bmp = new Bitmap(w, h))
            {
                ...
                if(rprm.RenderText)
                {
                    using(Graphics g = Graphics.FromImage(bmp))
                    {
                        ...
                        if (model3.cf != null)
                            g.DrawString($"{rprm.Comment}\ntime: {model3.cf.SimulationTime:0.000} s",
                                new Font("Tahoma", 40), Brushes.Black, rectf, format);
                        else g.DrawString($"test line 1\ntest line 2",
                            new Font("Tahoma", 40), Brushes.Black, rectf, format);
        ...
        private void renderSimulationToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            do
            {
                ...
                renderingFrame++;
            } while (renderingFrame < rprm.nFrames);


            // write 50 copies of the last frame
