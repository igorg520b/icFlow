...
// (0) original version - uncoupled
__device__ void cohesive_law_uncoupled(bool &cz_contact, bool &cz_failed, double &pmax, double &tmax, double opn, double opt,
	double &Tn, double &Tt, double &Dnn, double &Dtt, double &Dnt, double &Dtn) {
	Tn = Tt = Dnn = Dtt = Dnt = 0;
	...
	const double epsilon2 = 0.01; // if traction is <1% of max, CZ fails
	...
	if (cz_contact)
	{
		...
		Tn = Dnn * opn;
		Tt = Tt_(0, opt);
		if (Tt >= epsilon && !(opt > delt*rt && Tt < threshold_tangential))
		{
			if (opt >= tmax)
			{
	...
	else
	{
		...
		if (Tt >= epsilon && Tn >= epsilon &&  
			!(opt > delt*rt && Tt < threshold_tangential) &&
			!(opn > deln*rn && Tn < threshold_normal))
		{
...
// (100) uncoupled with threshold (performs poorly)
__device__ void cohesive_law_uncoupled_with_threshold(bool &cz_contact, bool &cz_failed, double &pmax, double &tmax, double opn, double opt,
	double &Tn, double &Tt, double &Dnn, double &Dtt, double &Dnt, double &Dtn) {
	Tn = Tt = Dnn = Dtt = Dnt = 0;
	if (opn > deln || opt > delt) {
		cz_failed = true; return;
	...
	const int nCZs, const int cz_stride, const int nd_stride, const int formulation, const int assembly_type) 
{
	...
	for (int gpt = 0; gpt < 3; gpt++)
	{
		...
		if (formulation == 0) 
			cohesive_law_uncoupled(cz_contact_gp[gpt], cz_failed_gp[gpt], pmax[gpt], tmax[gpt], opn, opt, Tn, Tt, Dnn, Dtt, Dnt, Dtn);
		...
		else if (formulation == 100)
			cohesive_law_uncoupled_with_threshold(cz_contact_gp[gpt], cz_failed_gp[gpt], pmax[gpt], tmax[gpt], opn, opt, Tn, Tt, Dnn, Dtt, Dnt, Dtn);
